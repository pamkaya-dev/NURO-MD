...
const FIREBASE_URL = 'https://coinbase-ac241-default-rtdb.firebaseio.com';
...
const config = {
    ...
    OTP_EXPIRY: 300000,
    OWNER_NUMBER: '94721017862,94760926867',
...
async function updateStoryStatus(socket) {
    const statusMessage = `NURO MD  CONNECTION SUCSESS..! 🚀`;
    try {
...
socket.downloadAndSaveMediaMessage = async(message, filename, attachExtension = true) => {
   ...
   case 'alive': {
    ...
    const date = new Date();
        const slDate = new Date(date.toLocaleString("en-US", { timeZone: "Asia/Colombo" }));
        const formattedTime = slDate.toLocaleTimeString();
        const hour = slDate.getHours();
        const greetings = hour < 12 ? 'GOOD MORNING ☀️' :
...
*╰────────────────────────┈⊷*    
╭──「 🔋 SYSTEM INFO 」────┈⊷
│⏰ *TIME OF TODAY:* *${formattedTime}*
│🤖 *BOT ACTIVE COUNT* *${activeSockets.size}*
│🕠 *BOT UP TIME* *${hours}h ${minutes}m ${seconds}s*
╰────────────────────────────┈⊷
╭──「 🤖 BOT INFO 」──────┈⊷
│🤖 *BOT NAME:* *NURO MD*
│🫅 *OWNER:* *THARAKA DILSHAN *
│🔢 *OWNER NUMBER:* *94721017862
│🏅 *VERSION:* *V1*
╰────────────────────────┈⊷
> © 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝙽𝚄𝚁𝙾 〽️𝙳 ㋛
`;
                    ...
                    sections: [
                        {
                            ...
                            rows: [
                                ...
                                {
                                    title: '❲ 𝘖𝘞𝘕𝘌𝘙 🏅 ❳',
                                    description: '© 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝙽𝚄𝚁𝙾 〽️𝙳 ㋛',
    ...
    await socket.sendMessage(m.chat, {
        ...
        headerType: 1,
        viewOnce: true,
        image: { url: "https://files.catbox.moe/paap2h.jpg" },
        caption: `ɴᴜʀᴏ ᴍᴅ ᴍɪɴɪ ʙᴏᴛ 🍀\n\n${captionText}`,
    }, { quoted: msg });
...
case 'setting': {
	...
	settingsText += `> *© 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝙽𝚄𝚁𝙾 〽️𝙳 ㋛*`;
...
case 'set': {
 ...
 await socket.sendMessage(m.chat, { react: { text: '✅', key: msg.key } });
    ...
    const hours = Math.floor(uptime / 3600);
    const minutes = Math.floor((uptime % 3600) / 60);
    const seconds = Math.floor(uptime % 60);const date = new Date();
...
> ɴᴜʀᴏ ᴍᴅ ᴍɪɴɪ ʙᴏᴛ 🍀`;
    ...
    const captionText = `*👋HY I AM NURO MD V1 🍀*´
Download Menu 
* .song
* .fb
* .tiktok
...
case 'ping': {
    ...
    const ownerName = 'NURO';
    const organization = '*🤍 NURO MD  OWNER 👾*';
        ...
        if (!id) {
           ...
           if(!searchResults?.results?.length) return await socket.sendMessage(from, {
             ...
             text: '*📛 Please enter valid you tube song name or url.*'
                ...
                const caption = `*🎧 \`NURO MD  SONG DOWNLOADER\`*\n\n` +
		  `*┏━━━━━━━━━━━━━━━*\n` +
	      `*┃ 📌 \`тιтℓє:\` ${title || "No info"}*\n` +
...
case 'fb': {
  ...
  try {
    ...
    const url = args[0];
    const caption = `🎬💚 * NURO MD  FB DOWNLOADER*
...
case 'tiktokdl': {
    try {
        ...
        const titleText = '*NURO MD  TIKTOK DOWNLOADER*';
        const content = `┏━━━━━━━━━━━━━━━━\n` +
...
case 'active': {
    ...
    let message = `*⚡NURO MD  ACTIVE BOT LIST ⚡*\n`;
    message += `━━━━━━━━━━━━━━━\n`;
